...
- **LLM**: Google Gemini 2.5 Flash (Experimental)
...
### ⚠️ Director.py 수정 금지 (Title Block 관련)

**CRITICAL**: `backend/app/tasks/director.py`의 **Title Block 렌더링 로직은 절대 수정 금지**입니다.

#### 수정 금지 구간 (Lines 249-400)
- Title block configuration logging (lines 253-260)
- `create_title_block_clips()` 함수 (lines 262-325)
- Title block rendering logic (lines 387-400)

#### 왜 수정하면 안되나요?
- Title block 렌더링은 MoviePy 2.x의 복잡한 제약사항을 우회하기 위해 정교하게 설계됨
- 한글 텍스트 + stroke + 멀티라인 지원을 위한 height 계산이 매우 까다로움
- 과거에 여러 번 수정했다가 title block이 렌더링 안되는 문제 반복 발생
- **한 번 작동하면 절대 건드리지 말 것!**

#### 허용되는 수정
- Subtitle 렌더링 로직 (lines 402-500): 자막 관련 수정은 가능
- Scene composition 로직 (lines 330-380): 이미지 레이어링 수정 가능
- Audio sync 로직 (lines 500-600): 오디오 관련 수정 가능

#### 만약 Title Block에 문제가 생기면?
1. **절대 director.py를 수정하지 마세요**
2. 대신 frontend 또는 layout.json 생성 로직을 수정하세요
3. Title block 스타일 변경은 `layout_config`를 통해서만 하세요

---

### ⚠️ Celery Worker 재시작 필수 사항

**CRITICAL**: Celery Worker는 Python 코드를 메모리에 캐싱하기 때문에, `backend/app/` 폴더의 **어떤 Python 파일이든 수정하면 반드시 Worker를 재시작**해야 합니다.

#### 재시작이 필요한 경우
- `app/tasks/` 폴더의 모든 파일 (plan.py, designer.py, composer.py, voice.py, director.py, qa.py 등)
- `app/utils/` 폴더의 모든 파일 (plot_generator.py, json_converter.py 등)
- `app/providers/` 폴더의 모든 파일
- `app/schemas/` 폴더의 모든 파일
- **요약: backend/app/ 하위의 모든 .py 파일**

#### 재시작 방법
```bash
# 1. 모든 Celery Worker 프로세스 종료
pkill -f "celery.*worker"

# 2. Worker가 완전히 종료되었는지 확인
...
#### 디버깅 팁
- 코드 수정 후에도 로그에 변경사항이 보이지 않으면 → Worker가 재시작되지 않은 것
- 새로운 로깅 구문이 출력되지 않으면 → Worker가 여전히 이전 코드를 실행 중
- 항상 Worker 재시작 후 **새로운 run_id로 테스트**해야 함 (이전 run_id는 캐시된 결과 사용 가능)
